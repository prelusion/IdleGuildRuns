...
const QUALITY = {
  common: { label: "Common", ring: "from-zinc-300 via-zinc-100 to-zinc-400", glow: "rgba(245,245,245,0.30)", accent: "text-zinc-100", muted: "text-zinc-300/80" },
  uncommon: { label: "Uncommon", ring: "from-emerald-400 via-emerald-200 to-emerald-500", glow: "rgba(52,211,153,0.30)", accent: "text-emerald-100", muted: "text-emerald-200/80" },
  rare: { label: "Rare", ring: "from-sky-500 via-sky-200 to-sky-600", glow: "rgba(56,189,248,0.28)", accent: "text-sky-100", muted: "text-sky-200/80" },
  epic: { label: "Epic", ring: "from-fuchsia-500 via-violet-300 to-fuchsia-700", glow: "rgba(217,70,239,0.28)", accent: "text-fuchsia-100", muted: "text-fuchsia-200/80" },
  legendary: { label: "Legendary", ring: "from-amber-500 via-yellow-200 to-orange-600", glow: "rgba(251,191,36,0.28)", accent: "text-amber-100", muted: "text-amber-200/80" },
} as const;

const PRIMARY_LABELS: Record<keyof NonNullable<Accessory["primaryStats"]>, string> = {
  stamina: "STA",
  strength: "STR",
  intellect: "INT",
  agility: "AGI",
...
const SECONDARY_LABELS: Record<keyof NonNullable<Accessory["secondaryStats"]>, string> = {
  leech: "Leech",
  speed: "Speed",
  haste: "Haste",
  crit: "Crit",
...
function hasStats(x: Item | Gear | Weapon | Accessory): x is Gear | Weapon | Accessory {
  return "primaryStats" in x && "secondaryStats" in x;
}

function sumResists(r: Accessory["secondaryStats"]["resistances"]) {
  return r.fireResistance + r.frostResistance + r.arcaneResistance + r.shadowResistance + r.poisonResistance + r.stunResistance;
}
  ...
  return (
    <div className={"w-[230px] top-10 right-90 z-50 " + customClass}>
      ...
      <div className="relative rounded-2xl p-[2px]">
        {/* Outer gradient ring */}
        <div className={["absolute inset-0 rounded-2xl bg-gradient-to-br", q.ring, "opacity-90"].join(" ")} />
        ...
        >
          ...
          <div className="flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="shrink-0 flex justify-end">
                ...
                <span className="inline-flex items-center rounded-full px-2 py-1 text-[10px] font-semibold bg-white/5 border border-white/10 text-white/75">
                  {isWeapon(item) ? "Weapon" : isGear(item) ? `${item.type.toUpperCase()} • ${item.rank.toUpperCase()}` : "Item"}
                </span>
              </div>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className={["text-base font-semibold leading-5 ", q.accent].join(" ")}>{item.name}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className={"flex flex-col w-full items-center justify-center"}>
            <div className={["text-xs mt-1", q.muted].join(" ")}>
              Value: <span className="font-semibold text-white/90 tabular-nums">{item.value}</span> · Gold:{" "}
              <span className="font-semibold text-white/90 tabular-nums">{item.monetaryValue}</span>
            </div>
                ...
                {isWeapon(item) && (
                  <>
                    <span className="tabular-nums">
                      DMG: <span className="text-white/90 font-semibold">{item.fromDamage}-{item.toDamage}</span>
                    </span>
                    <span className="tabular-nums">
                      Parry: <span className="text-white/90 font-semibold">{item.parry}</span>
                    </span>
                    <span className="tabular-nums">
                      Block: <span className="text-white/90 font-semibold">{item.block}</span>
                    </span>
                    <span className="tabular-nums">
                      Range: <span className="text-white/90 font-semibold">{item.range}</span>
                    </span>
                    <span className="tabular-nums">
                      Armor: <span className="text-white/90 font-semibold">{item.armor}</span>
                    </span>
                  </>
                )}
          ...
          {primary && secondary && (
            <div className="grid grid-rows-1 gap-3">
              <div className="pt-2">
                <div className="text-[10px] uppercase tracking-wider text-white/50 mb-2">Primary</div>
                <div className="space-y-1 rounded-xl bg-white/5 border border-white/10 p-2 grid grid-cols-2 gap-x-4">
                  {(Object.keys(PRIMARY_LABELS) as Array<keyof typeof PRIMARY_LABELS>).map((k) => (
              ...
              <div className="">
                <div className="flex items-baseline justify-between">
                  <div className="text-[10px] uppercase tracking-wider text-white/50 mb-2">Secondary</div>
                  <div className="text-[10px] text-white/45">
                    Total Resists: <span className="font-semibold text-white/75 tabular-nums">{sumResists(secondary.resistances)}</span>
                  </div>
                </div>
